...
%{
  ...
  # Разворачивается только 6-max: за девятью местами 36 карт делятся плохо,
  # а хедз-ап на анте кнопки вырождается. Это решение сетки, а не правило —
  # запрета на другие форматы Short Deck в коде нет.
  formats: [
