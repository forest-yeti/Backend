...
```bash
cd /opt/block_poker/src
sudo bash deploy/update.sh --pull
```
...
| Флаг | Что делает |
|---|---|
| *(без флагов)* | код, миграции, сид обеих сеток |
| `--pull` | сначала `git fetch` + `reset --hard` на текущую ветку |
...
Скрипт скопирует их в `/opt/block_poker/src`, не тронув `_build` и `deps`, —
в них весь смысл быстрого обновления: зависимости и NIF уже собраны под эту
машину.
