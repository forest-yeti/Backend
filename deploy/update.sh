...
# --------------------------------------------------------------------------
# 1a. Недостающие переменные окружения
# --------------------------------------------------------------------------
#
# `ubuntu.sh` переписывает env-файл целиком, `update.sh` не трогает его
# вовсе. Из-за этого нода, развёрнутая до появления какой-либо переменной,
# без неё и остаётся: обновление кода само по себе новых ключей не
# приносит. Дописываем недостающее здесь — идемпотентно, только то, чего
# в файле нет, и ничего не переписывая.
#
# Для автообновления клиента это критично: без `CLIENT_FEED_URL` ручка
# версии отдаёт `feed_url: null`, и клиент честно не запускает проверку
...
# на первом же ключе, которого в файле нет, — то есть ровно тогда, когда
# функция и должна сработать.
add_env() {
  local key="$1" value="$2"
  ...
  log "Добавляю ${key} в $ENV_FILE"
  printf '%s=%s
' "$key" "$value" >> "$ENV_FILE"
}

PHX_HOST_NOW="$(sed -n 's/^PHX_HOST=//p' "$ENV_FILE")"

if [[ -n "$PHX_HOST_NOW" ]]; then
  # Схему берём по факту наличия сертификата, а не по предположению:
  # ошибиться здесь значит выдать клиентам адрес фида, по которому
  # ничего не отвечает.
  if [[ -d "/etc/letsencrypt/live/${PHX_HOST_NOW}" ]]; then
    UPDATES_SCHEME="https"
  else
    UPDATES_SCHEME="http"
  fi

  # Слеш в конце обязателен: `electron-updater` резолвит `latest.yml`
  # относительно этого адреса через `new URL/2`, а без слеша такое
  # разрешение отбрасывает последний сегмент пути.
  add_env CLIENT_FEED_URL "${UPDATES_SCHEME}://${PHX_HOST_NOW}/client-updates/"
else
  warn "в $ENV_FILE нет PHX_HOST — CLIENT_FEED_URL не добавлен, автообновление останется выключенным"
fi

add_env CLIENT_UPDATES_DIR "$UPDATES_DIR"
